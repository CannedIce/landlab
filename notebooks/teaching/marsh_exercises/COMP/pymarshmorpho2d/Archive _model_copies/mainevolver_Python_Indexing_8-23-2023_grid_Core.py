...
class mainEvolution(Component):
    ...
    def bedcreepponds(self, dx, dt):
        ...
        pReindexed = np.zeros(len(self._elev)) # reindex for p being potentially longer than the masked data and causing matrix index overruns
        pReindexed[:] = np.nan
        pReindexed[p] = range(0,len(self._index[p]))
        ...
        for k in [N, -1, 1, -N]: # calculate the gradianets between cells in the x and y direction
            ...
            N, M = self._grid.shape
            row, col = np.unravel_index(tmp, shape=(N-2, M-2)) # sort this out.
            ...
            # There is an issue here on the 6th loop? Seems to crash.
            # only include cells that can creep
            for i in range(len(parray[a])):
                try:
                    if a[i] == True and self._model_domain[q[a][i]] != 1:
                        a[i] = False
                except:
                    a[i] = False
                    # print("Stupid stuff is happening")
            # foo = np.where(self._model_domain > -9999, True, False)
            # a[self._model_domain[q[a]] == 1] = True
            ...
            s = list(s) + list(-value)
            lenTrack += len(parray[a])
        ilog = list(ilog) + list(nc)
        jlog = list(jlog) + list(nc)
        # build a list of values exiting the node
        s = list(s) + list(1 + S[nc])
        lenTrack += len(parray[a])
        ds2 = sparse.csc_array((s, (ilog, jlog)), shape=(max(row)*max(col), max(row)*max(col)))
        try:
    ...
    def update_morphology(self, dt):
        ...
        #######################################################################################################
        # # bedcreepponds
        # from bedcreepponds import bedcreepponds
        # foo = bedcreepponds(self)
        ...
        self._Qs =E / (self._ws2 * 3600 * 24) * self._U * np.maximum(0,self._water_depth) # kg/m/s %[hcorrected]=getwaterdepth(Trange,Hsurge,msl,z,kro,hpRIV);  %VEG=(z-msl)>dBlo;
