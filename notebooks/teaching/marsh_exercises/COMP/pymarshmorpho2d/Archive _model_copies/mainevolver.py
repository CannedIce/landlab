...
class mainEvolution(Component):
    ...
    def update_flow(self):
        ...
        self._Ux[gridNum[-2, :]] = self._Ux[gridNum[-1, :]]
    ...
    def update_morphology(self, dt):
        ...
        for k in [N, -1, 1, -N]:
            ...
            row, col = np.unravel_index(self._index, shape=(N, M)) # sort this out.
            ...
            ilog = list(ilog) + list(matLabIndex[q[a]])
            jlog = list(jlog) + list(matLabIndex[parray[a]])
        ...
        P = scipy.sparse.linalg.spsolve(ds2, rhs.T, use_umfpack = True, permc_spec = "MMD_ATA") # was working with .lsqr
