...
class mainEvolution(Component):
    ...
    def update_morphology(self, dt):
        ...
        # calculate the delta of flow into an away from a node.
        from landlab.grid.raster_mappers import map_sum_of_inlinks_to_node, map_sum_of_outlinks_to_node
        inNode = map_sum_of_inlinks_to_node(self._grid, self._flood_tide_vel)
        outNode = map_sum_of_outlinks_to_node(self._grid, self._flood_tide_vel)

        dx = 5 * 2
        # (self._DoMUD * 24 * 3600 + self._DiffS * self._tidal_period / 2 * np.abs(self._Ux * self._Ux) * (24 * 3600) ** 2) / (dx ** 2) * self._water_depth
        DinNode = (self._DoMUD * 24 * 3600 + self._Diffs * self._tidal_period / 2 * np.abs(inNode * inNode) * (24 * 3600) ** 2) / (dx ** 2) * self._water_depth
        DoutNode = (self._DoMUD * 24 * 3600 + self._Diffs * self._tidal_period / 2 * np.abs(outNode * outNode) * (24 * 3600) ** 2) / (dx ** 2) * self._water_depth

        #DD = DinNode + DoutNode/2
        valuein = DinNode / self._water_depth / self._hydroperiod
        valueout = DoutNode / self._water_depth / self._hydroperiod
        ...
        settling = 24 * 3600 * WS / self._water_depth * self._hydroperiod
        ...
        # ilog = list(ilog) + list(matLabIndex[p])
        # jlog = list(jlog) + list(matLabIndex[p])
        ...
        # ds2 = np.reshape(ds2 ,(500,300))
        # rhs = np.reshape(rhs, (500,300))
        # ds3 = ds2 #np.vstack([ds2[:,1:2], np.zeros(self._grid.shape[0])])
        rhs2 = rhs[:,2]
        # ds3 = scipy.sparse.csc_matrix(ds2, shape= (500,300))
        P = scipy.linalg.lstsq(ds2, rhs)[0] # was working with .lsqr
        # vals = P[0] * ds3 + P[1]
    ...
    def run_one_step(self, timeStep, round, model_domain):
        """Advance in time."""
        # Update sea level
        self._mean_sea_level = self._mean_sea_level +  (self._rel_sl_rise_rate * timeStep)
